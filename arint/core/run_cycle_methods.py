# core/run_cycle_methods.py
"""
Helper methods untuk run_cycle yang sudah terintegrasi dengan Flock of Thought.
Methods ini akan di-assign ke class SilentWatcherSuper.
"""
...
logger = logging.getLogger("RunCycleMethods")
...
def _execute_run_evolution(self):
    """Execute RUN_EVOLUTION_CYCLE action."""
    self.log("Initiating evolutionary optimization cycle...")
    try:
        if hasattr(self, "evolver") and self.evolver is not None:
            result = self.evolver.evolve_cycle()
            self.needs["fatigue"] = min(100, self.needs["fatigue"] + 20)
            return f"Evolution cycle result: {str(result)[:100]}", True
        else:
            return "Evolution engine not available.", False
    except Exception as e:
        self.log(f"Evolution error: {e}", level="ERROR")
        return f"Evolution failed: {e}", False
